...
while True:
    print("\n1. 상태 \n 2. 위치 \n 3. 시간 \n 4. 수업 \n 5. 이동 \n 6. 난이도 변경 \n 7. 가방 \n 8. 종료")
    answer = input("Please select your option: ")
    
    if answer == "1" or answer == "상태":
        print("당신의 상태는 '배고픔'입니다.")
        print(f"현재 게임 난이도: {settings['난이도']}")
        
        status_choice = input("어떤 상태를 확인하시겠습니까? (1. 계좌, 2. HP): ")
        ...
        if not inventory:
            ...
            for i, item in enumerate(inventory):
                print(f"{i+1}. {item}")
            choice = input("사용할 물건의 이름 또는 번호를 입력하세요 (취소: 엔터): ")
            if choice:
                usable_item = None
        ...
        print("당신의 시간은 '11시'입니다.")
    elif answer == "4":
        print("당신의 수업은 1시 이윤재관 511호에 있습니다.")
    elif answer == "5":
        while True:
            direction = input("이동할 방향을 입력하세요 (동, 서, 남, 북) 또는 '그만'을 입력하여 돌아가기: ")
            ...
            if 0 <= new_row < len(map_data) and 0 <= new_col < len(map_data[0]) and map_data[new_row][new_col] != "":
                ...
                if curr_place == "학생회관":
                    ...
                    while True:
                        buy_choice = input("무엇을 구매하시겠습니까? (1. 두쫀쿠(5000원), 2. 카페라떼(2500원), 3. 나가기): ")
        ...
        diff = input("변경할 난이도를 선택하세요 (쉬움, 보통, 어려움): ")
        if diff in ["쉬움", "보통", "어려움"]:
            settings["난이도"] = diff
