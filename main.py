...
def load_events():
    ...
    file_path = "events.pkl"
    if os.path.exists(file_path):
        with open(file_path, "rb") as f:
            data = pickle.load(f)
            
        if "events" in data:
            dynamic_events.update(data["events"])
            
        if "answers" in data:
            answers = data["answers"]
            if "교내 부조리 수사" in answers:
                bujo_answers.append(answers["교내 부조리 수사"])
            if "교내 위생사건 수사" in answers:
                food_answers.append(answers["교내 위생사건 수사"])
...
def 초기_설정():
    ...
    print("● 난이도별 설정:")
    print("  - 쉬움: 한 칸 이동할 때마다 HP가 0.5만큼 감소한다.")
    print("  - 보통: 한 칸 이동할 때마다 HP가 1만큼 감소한다.")
    print("  - 어려움: 한 칸 이동할 때마다 HP가 2만큼 감소한다.")
    while True:
